define('page/menu', ['crud'], function(CRUD) {
	var menu = {
		grid: null,
		init: function() {
			var the = this;
			this.grid = $('#menugrid').datagrid({
				url: 'menu/paged',
				fit: true,
				rownumbers: true,
				pagination: true,
				columns: [[{
					...
					field: 'url', title: '入口地址', width: 150
				}, {
				...
				toolbar: [{
					...
					handler: function() {
						...
						if (CRUD.onlyCheckedOne(checkRow)) {
							CRUD.edit({
								title: '编辑菜单--' + checkRow[0].name,
								href: 'menu/update?id=' + checkRow[0].id
							}, function() {
								the.loaded();
							}, function() {
								the.saveOrUpdate();
