...
public class RefundService {
	public void refund(RefundOrderInfo refundOrderInfo){
		//把请求参数打包成数组
...
//				sParaTemp.put("notify_url", notify_url);//非必填
				sParaTemp.put("seller_email", AlipayConfig.seller_email);//卖家支付宝账号,如果卖家Id已填，则此字段可为空。
				sParaTemp.put("seller_user_id", AlipayConfig.seller_user_id);//卖家支付宝账号对应的支付宝唯一用户号。以2088开头的纯16位数字。登录时，seller_email和seller_user_id两者必填一个。如果两者都填，以seller_user_id为准。
				sParaTemp.put("refund_date", UtilDate.getDateFormatter());//退款请求的当前时间。格式为：yyyy-MM-dd hh:mm:ss。
				sParaTemp.put("batch_no", UtilDate.getBatchNo());//退款批次号
				sParaTemp.put("batch_num", String.valueOf(refundOrderInfo.getOrder().size()));//退款总笔数，即参数detail_data的值中，“#”字符出现的数量加1
	...
	public static String createDetailData(RefundOrderInfo refundOrderInfo){
		...
		List<Order> orders = refundOrderInfo.getOrder();
		for(Order o:orders){
